...
class AgentCareController extends Controller
{
    ...
    public function userDetailsPaidProfit(Request $request, $username)
    {
        ...
        try {
            AgentProfit::where('agent_id', $user->id)->where('paid', '0')->update([
                'paid' => $request->target,
                'admin_id' => $admin->id,
                'date_paid' => now(),
            ]);
        ...
        return back()->with(['success' => [__("Agent profits Updated Successfully!")]]);
    }
    ...
    public function walletBalanceUpdate(Request $request, $username)
    {
        ...
        // 27/08/2024
        $basic_setting = BasicSettings::first();
        if ($basic_setting->plafond_collect < $validated['amount']) return back()->with(['error' => [__("Amount must be under 300 000 XAF!")]]);
        ...
        $hold = Transaction::where('agent_id', $user_wallet->agent->id)->where('type', PaymentGatewayConst::MONEYIN)
            ->where('cbsTransfert', 'ND')
            ->where('status', 1)
            ->count();
        if ($hold > 0) return back()->with(['error' => [__("You have to validated all pending collect for this agent before add momey to his balance!")]]);
    ...
    public function autoWalletBalanceUpdate()
    {
        $users_wallet = AgentWallet::get()->toArray();

        foreach ($users_wallet as $row => $user_wallet) {
            DB::beginTransaction();
            try {
                $inserted_id = DB::table("transactions")->insertGetId([
                    'admin_id'          => auth()->user()->id,
                    ...
                    'trx_id'            => generate_unique_string("transactions", "trx_id", 16),
                    'request_amount'    => $user_wallet['balance'],
                    'payable'           => $user_wallet['balance'],
                    'available_balance' => 0,
                    'remark'            => 'Auto Balance Substract',
                    'status'            => GlobalConst::SUCCESS,
                    'created_at'        => now(),
                ]);
                ...
                DB::table('agent_wallets')->where('id', $user_wallet['id'])->update([
                    'balance' => 0,
                ]);
                DB::commit();
            } catch (Exception $e) {
    ...
    {
        $event = new AgentCoordinatesUpdated();
        event($event);
    }
}
