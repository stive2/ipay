...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        //agent profits
        $agent_profits = TransactionCharge::toBase()->sum('total_charge');

        $this_month_agents_profits = TransactionCharge::toBase()
            ->whereBetween('created_at', [$this_month_start, $this_month_end])
            ->sum('total_charge');

        $last_month_agents_profits = TransactionCharge::toBase()
            ->whereBetween('created_at', [$last_month_start, $last_month_end])
            ->sum('total_charge');
        ...
        if ($pending_money_in == 0 && $completed_money_in != 0) {
            $money_in_percent = 100;
        } elseif ($pending_money_in == 0 && $completed_money_in == 0) {
            $money_in_percent = 0;
        } else {
            $money_in_percent = ($completed_money_in / ($completed_money_in + $pending_money_in)) * 100;
        }
        ...
        //Agents
        $total_agents = Agent::toBase()->count();

        $active_agents =  Agent::active()->count();
        $unverified_agents = Agent::smsUnverified()->count();

        if ($unverified_agents == 0 && $active_agents != 0) {
            $agent_perchant = 100;
        } elseif ($unverified_agents == 0 && $active_agents == 0) {
            $agent_perchant = 0;
        } else {
            $agent_perchant = ($active_agents / ($active_agents + $unverified_agents)) * 100;
        }
        ...
        // Money In
        $Money_in_pending_data  = [];
        $Money_in_success_data  = [];
        $Money_in_canceled_data = [];
        $Money_in_hold_data     = [];
        ...
        while ($start <= $end) {
            ...
            $money_hold = Transaction::where('type', PaymentGatewayConst::MONEYIN)
                ...
                ->where('status', 3)
                ->count();
            $Money_in_pending_data[]  = $money_pending;
            $Money_in_success_data[]  = $money_success;
            $Money_in_canceled_data[] = $money_canceled;
            $Money_in_hold_data[]     = $money_hold;
        ...
        // Chart three
        $chart_three_data = [
            'pending_data'  => $Money_in_pending_data,
            'success_data'  => $Money_in_success_data,
            'canceled_data' => $Money_in_canceled_data,
            'hold_data'     => $Money_in_hold_data,
        ];
        ...
        $total_agent = Agent::toBase()->count();
        $unverified_agent = Agent::toBase()->where('sms_verified', 0)->count();
        $active_agent = Agent::toBase()->where('status', 1)->count();
        $banned_agent = Agent::toBase()->where('status', 0)->count();
        $chart_agent = [$active_agent, $banned_agent, $unverified_agent, $total_agent];
        ...
        $data = [
            ...
            'completed_money_in'      => $completed_money_in,
            'pending_money_in' => $pending_money_in,
            'money_in_percent'    => $money_in_percent,
