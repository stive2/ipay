...
class UserCareController extends Controller
{
    ...
    public function kycApprove(Request $request, $username)
    {
        ...
        if ($request->rib != $request->rib_confirmation) return back()->with(['error' => ['RIB are not the same']]);
        // 20/08/2024
        $usert = User::where('rib', $request->rib)->first();
        if ($usert) return back()->with(['error' => ['RIB is already exist']]);

        $ribVerified = false;

        try {
            if ($basic_setting->rib_config != null) {
                if ($basic_setting->rib_config->active == 'yes') {
                    $data = [
                        'rib' => $request->rib,
                    ];
                    if (GlobalController::verify_rib($data) != false) {
                        $ribVerified = true;
                    }
                } else {
                    $ribVerified = true;
        ...
        if ($ribVerified) {
            ...
            return back()->with(['error' => [__("The RIB don't exist.")]]);
        }
    ...
    public function walletBalanceUpdate(Request $request, $username)
    {
        ...
        try {
            ...
            if ($user_wallet->user->type == 'email') {
