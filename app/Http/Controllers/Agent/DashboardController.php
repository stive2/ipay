...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $test1 = Transaction::agentAuth()->addSubBalance()->where('status', 1)->orderBy('created_at', 'desc')->first();
        if ($test1) {
            $data['lastAddSubBalance'] = amountOfTransaction($test1);
        } else $data['lastAddSubBalance'] = 0;
        $data['totalAddSubBalance'] = Transaction::agentAuth()->addSubBalance()->where('status', 1)->sum('request_amount');
        ...
        $data['total_transaction'] = Transaction::agentAuth()->moneyIn()->where('status', 1)->count();
        $data['agent_profits'] = AgentProfit::agentAuth()->where('paid', '0')->sum('total_charge');
        ...
        while ($start <= $end) {
            ...
            // Monthley add money
            $pending = Transaction::agentAuth()->where('type', PaymentGatewayConst::MONEYIN)
                ->whereDate('created_at', $start_date)
                ->where('status', 2)
                ->count();
            $success = Transaction::agentAuth()->where('type', PaymentGatewayConst::MONEYIN)
                ->whereDate('created_at', $start_date)
                ->where('status', 1)
                ->count();
            $canceled = Transaction::agentAuth()->where('type', PaymentGatewayConst::MONEYIN)
                ->whereDate('created_at', $start_date)
                ->where('status', 4)
                ->count();
            $hold = Transaction::agentAuth()->where('type', PaymentGatewayConst::MONEYIN)
                ->whereDate('created_at', $start_date)
                ->where('status', 3)
                ->count();
            ...
            // Monthley money Out
            $money_pending = Transaction::agentAuth()->where('type', PaymentGatewayConst::TYPEADDSUBTRACTBALANCE)
                ->whereDate('created_at', $start_date)
                ->where('status', 2)
                ->count();
            $money_success = Transaction::agentAuth()->where('type', PaymentGatewayConst::TYPEADDSUBTRACTBALANCE)
                ->whereDate('created_at', $start_date)
                ->where('status', 1)
                ->count();
            $money_canceled = Transaction::agentAuth()->where('type', PaymentGatewayConst::TYPEADDSUBTRACTBALANCE)
                ->whereDate('created_at', $start_date)
                ->where('status', 4)
                ->count();
            $money_hold = Transaction::agentAuth()->where('type', PaymentGatewayConst::TYPEADDSUBTRACTBALANCE)
                ->whereDate('created_at', $start_date)
                ->where('status', 3)
                ->count();
